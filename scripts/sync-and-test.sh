...
show_help() {
    ...
    echo -e "  -b, --rebuild-ipxe        ${I18N_REBUILD_DESC}"
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --rebuild-ipxe|-b|rebuild) REBUILD_IPXE="on" ;;
...
echo -e "${BLUE}${I18N_ST_TITLE}${NC}"
echo -e "${BLUE}${I18N_ST_TARGET_MODE}: ${TEST_MODE} | ${I18N_ST_LANG}: ${LANG_OPTION} | ${I18N_ST_FS}: ${FULLSCREEN_OPTION} | ${I18N_ST_REBUILD}: ${REBUILD_IPXE}${NC}"

# 1. Optionally Rebuild iPXE binaries FIRST (if explicitly requested via --rebuild-ipxe)
if [[ "$REBUILD_IPXE" == "on" ]]; then
    echo -e "${BLUE}${I18N_ST_REBUILDING_IPXE}${NC}"
    if ! bash "${SCRIPT_DIR}/build_ipxe_binaries.sh"; then
        echo -e "${RED}Error: iPXE recompilation failed.${NC}"
        exit 1
    fi
fi
...
# 2. Build UniBoot.iso (always regenerate to ensure latest scripts and binaries are packed)
